...
class IntentLexicon {
  ...
  static const List<String> actionEndings = [
    ...
    // 공부/기획/스터디 계열 명사구("사주공부/타로공부/○○ 기획")도 할일로 안착.
    // someday("나중에 …공부하기")는 폴백에서 someday 가드로 걸러지므로 안전.
    '공부', '기획', '스터디',
    // 그 자체가 동작인 집안일 단독명사("설거지/분리수거"). 과거형("설거지 끝냈어")은
    // 이미 점수가 있어 폴백을 안 타므로 지금기록 유지.
    '설거지', '분리수거', '다림질', '걸레질',
  ];
  ...
  static const Map<IntentType, List<String>> primary = {
    IntentType.scheduleAdd: [
      '예약', '미팅', '약속', '회의', '모임', '병원', '치과',
      // 날짜가 붙는 이벤트 — 담기동사 없이 "30일 현이 생일"만으로도 일정으로.
      '생일', '생신', '기념일', '제사',
    ],
    ...
    // '사주/타로'는 주제어로도 흔해(사주공부·타로공부·사주웹앱 기획) 단독으론 안
    // 잡는다. 보는 동작과 함께일 때만 운세로(분류기 특례). '운세/별자리'는 단독으로도
    // 거의 점보기 뜻이라 여기 둔다.
    IntentType.helpFortune: ['운세', '오늘 운', '별자리'],
  };
