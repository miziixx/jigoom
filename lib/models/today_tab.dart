...
extension TodaySectionX on TodaySection {
  String get label {
    switch (this) {
      case TodaySection.stats:    return '통계 요약';
      case TodaySection.habits:   return '습관';
      case TodaySection.upcoming: return '이벤트/일정';
      case TodaySection.memos:    return '메모';
      case TodaySection.todo:     return '할일';
      case TodaySection.timelog:  return '시간일기';
    }
  }

  String get headerKey {
    switch (this) {
      case TodaySection.stats:    return 'STATS';
      case TodaySection.habits:   return 'HABITS';
      case TodaySection.upcoming: return 'UPCOMING';
      case TodaySection.memos:    return 'MEMOS';
      case TodaySection.todo:     return 'TODO';
      case TodaySection.timelog:  return 'TIMELOG';
    }
  }
}
...
class TodayTab {
  ...
  factory TodayTab.fromJson(Map<String, dynamic> json) {
    final rawSections = (json['sections'] as List<dynamic>?)?.cast<String>() ?? [];
    ...
    if (rawOrder != null) {
      final parsed = rawOrder.map(_parseSection).whereType<TodaySection>().toList();
...
List<TodayTab> defaultTodayTabs() => [
      ...
      TodayTab(
        id: 't1',
        name: '시간일기',
        sections: [TodaySection.timelog],
      ),
      TodayTab(
        id: 't2',
        name: '할일',
        sections: [TodaySection.todo],
      ),
    ];
