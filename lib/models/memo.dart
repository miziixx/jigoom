...
class Memo {
  ...
  final List<DateTime>
  editHistory; // timestamps of edits (creation not included)
  final DateTime? reminderAt; // scheduled notification time, null = no reminder
  final String
  reminderRepeat; // 'none' | 'daily' | 'weekly' | 'monthly' | 'yearly'
  final DateTime?
  scheduledAt; // appointment/schedule time (! shortcut), null = not scheduled
  ...
  Memo copyWith({
    ...
    scheduleRepeat: clearSchedule
        ? 'none'
        : (scheduleRepeat ?? this.scheduleRepeat),
  ...
  Map<String, dynamic> toJson() => {
    ...
    'folderId': folderId,
    'editHistory': editHistory.map((d) => d.toIso8601String()).toList(),
  ...
  factory Memo.fromJson(Map<String, dynamic> json) => Memo(
    ...
    appendNotes:
        (json['appendNotes'] as List<dynamic>?)
