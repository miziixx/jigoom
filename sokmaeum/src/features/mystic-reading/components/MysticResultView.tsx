...
export default function MysticResultView({ result: r, readingId, hasHour, premium = true, onUnlock }: Props) {
  return (
      ...
      {premium ? (
        <>
          ...
          <MysticCard title="돈이 붙는 방식과 새는 구멍" readingId={readingId} sectionKey="workAndMoney">
            ...
            <p>돈이 새는 구멍 — {r.workAndMoney.moneyLeakPattern}</p>
            <p>잘 맞는 일의 환경 — {r.workAndMoney.suitableWorkEnvironment}</p>
            <p>피해야 할 일의 환경 — {r.workAndMoney.unsuitableWorkEnvironment}</p>
          ...
          {r.partnerReading && (
            ...
            >
              ...
              <p>이 관계의 숨은 권력구도 — {r.partnerReading.powerDynamic}</p>
              <p>관계가 애매해지는 이유 — {r.partnerReading.ambiguityReason}</p>
              <p>관계 전환 가능 시기 — {r.partnerReading.transitionTiming}</p>
        ...
        <div className="card mystic-premium-preview">
          <span className="premium-badge">무료 맛보기</span>
          <h3>전체 속마음 리포트에서 이어지는 내용</h3>
          <p>
            반복되는 인생 패턴, 돈과 일의 흐름, 관계 속마음, 올해의 전환점, 지금 피해야 할 선택과 해야 할 선택은
            전체 리포트에서 열립니다.
          </p>
          <ul>
            <li>사주 근거와 심리 언어를 함께 정리한 상세 리딩</li>
            <li>상대방 생년월일을 넣었을 때 관계 대조 리딩</li>
            <li>이어 묻기 챗봇과 저장용 리포트 확장</li>
          </ul>
          {onUnlock && (
