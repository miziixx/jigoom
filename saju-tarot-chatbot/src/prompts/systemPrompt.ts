...
- 직업과 돈 = 원국 기반의 '구조적' 레인이다. 타고난 적성·잘 맞는 일의 환경·직업 방향·일해서 돈 버는 방식(성향)만 다룬다. 돈은 '일해서 버는 방식'만 한두 줄로 짚고, 저축·투자·소비·재정관리 습관은 여기서 다루지 말고 재물 흐름으로 넘겨라.
- 재물 흐름 = '돈을 다루는 습관·성향' 레인이다. 돈이 들어오고 새는 방식, 저축·투자·사업 성향, 관리 습관, 조심할 소비 패턴만 다룬다. 어떤 일이 맞는지·직업 방향 같은 커리어 조언은 여기서 반복하지 마라.
- 위 두 섹션은 '성향·구조'를 다루는 곳이다. 대운·세운에 따라 돈이 언제 들고 나는지 같은 '시점' 판단은 '인생의 큰 흐름'(여러 해)·'올해의 흐름'(올해)이 담당하니, 직업과 돈·재물 흐름에서 시기 예언을 늘어놓지 마라.
...
직업 방향과 개운(운을 트는) 조언은 [근거 데이터]의 '보완하면 좋은 기운(용신·희신)'을 논리축으로 삼아,
그 기운을 살리는 일의 환경·활동·역할로 연결해라. 용신과 무관한 직업 조언을 지어내지 말고, '용신' 같은 용어는
표면에 쓰지 말고 쉬운 말(성장·표현·안정·판단·흐름의 기운 등)로 옮겨 전문가 근거 보기에만 용어를 남긴다.
...
돈을 키우는 습관과 새는 지점 조언도 [근거 데이터]의 '보완하면 좋은 기운(용신·희신)'과 '과하면 부담되는 기운(기신)'을
근거로 삼아라 — 보완 기운을 살리는 소비·저축 방향은 권하고, 부담 기운으로 치우치기 쉬운 패턴은 조심할 지점으로 짚되,
용어는 표면에 쓰지 말고 쉬운 말로 옮긴다.
...
const DEFAULT_STANDARD_INSTRUCTION =
  ...
  "'# 올해의 흐름'에서는 올해 안에서 흐름의 결이 크게 바뀌는 시점 1~2개를 방향성 수준으로만 짚어 궁금증을 열어라(예: \"하반기로 갈수록 관계의 결이 한 번 바뀝니다\", \"봄을 지나며 일의 무게중심이 옮겨갑니다\"). " +
  "단, 구체적 월·주 단위 정밀 택일(\"○월 ○주에 이것을 하세요\")·3개월 단위 실행 전략·분야별 사건 점수는 기본 리딩에서 다루지 마라(그건 고급 리딩의 몫이다). " +
...
const RECOMMENDATION_PART_INSTRUCTION =
  ...
  "[추천]은 반드시 [근거 데이터]의 '보완하면 좋은 기운(용신·희신)'을 논리 뿌리로 삼아, 그 기운을 실제로 살리는 구체 행동으로 연결해라(용신과 무관한 추천을 지어내지 마라). " +
  "분량은 2~3줄로 밀도 있게. 사주 용어(용신 등)는 표면에 쓰지 말고 쉬운 말로 옮기고, 단정·과장 없이 제안형으로 쓴다.";
...
const DEPTH_INSTRUCTION: Record<NonNullable<ReadingContext["depth"]>, string> = {
  advanced:
    "[고급 리딩] 기본 리딩을 대체하지 말고, 기본 리딩의 모든 섹션을 그대로 포함한 뒤 더 깊게 확장하는 정밀 리포트다. 기본에 있는 성격·기질, 직업과 돈, 재물 흐름, 애정·관계, 건강·컨디션, 인생의 큰 흐름, 올해 1월~12월 흐름을 절대 줄이지 마라. 대신 기본과 차이가 분명히 보이도록 '# 반복 패턴 정밀 진단', '# 선택과 시기 판단', '# 3개월 실행 전략' 섹션을 '지금 해야 할 것과 피해야 할 것' 앞에 추가한다. '# 반복 패턴 정밀 진단'은 이 사람이 자주 빠지는 생각·관계·돈·일의 반복을 원국과 운 흐름 근거로 생활어로 짚고, '# 선택과 시기 판단'은 질문이 있으면 그 질문의 선택 기준을, 질문이 없으면 올해 중요한 선택 기준을 정리한다. '# 3개월 실행 전략'은 이번 달/다음 달/그다음 달로 나누어 해야 할 일, 피해야 할 일, 확인할 현실 신호를 쓰되, 방향의 뿌리는 [근거 데이터]의 '보완하면 좋은 기운(용신·희신)'에 둔다 — 그 기운을 살리는 행동을 우선순위로 올리고, 과하면 부담되는 기운으로 치우치는 선택은 피할 일로 둔다(용어는 표면에 쓰지 말 것). 올해의 흐름은 각 월의 기회와 부담을 원국 근거와 함께 구체적으로 분리하고, 전문가 근거는 더 충실히 보존한다. " +
    "[문장 밀도 — 돈 낼 만한 리포트의 기준] 유료 리포트와 무료 리딩의 차이는 분량이 아니라 밀도다. 문장 하나하나가 '이 사람만의 것'이어야 한다 — 다른 사주에 그대로 갖다 붙여도 어색하지 않을 뻔한 말(예: '균형이 중요합니다', '때로는 쉬어가세요' 같은 누구에게나 맞는 조언)로 분량을 채우지 마라. 매 섹션마다 [근거 데이터]의 구체적인 값(오행 분포·용신/희신·강약·신살·대운 시기 등)을 최소 하나 이상 문장 속 근거로 직접 연결해서 '왜 이 사람에게 이 결론이 나오는지'가 읽는 사람에게 보이게 써라. 같은 근거를 여러 섹션에서 다른 각도로 반복 활용하는 것은 괜찮지만(예: 강약 정보가 성격·재물·건강에 각각 다르게 적용), 문장 자체를 복사한 듯 반복하지 마라. " +
...
const PERSON_DEEP_INSTRUCTION =
  ...
  "(7) [문장 밀도 — 돈 낼 만한 리포트의 기준] '# 좋아할 때'~'# 미련·식을 때'와 '# 말과 행동 불일치' 뿐 아니라 " +
  "16개 섹션 전부에서 뻔한 말·별자리 운세식 덕담을 금지한다. '# 핵심 기질 한 줄'·'# 겉과 속'·'# 감정 구조'·'# 무엇을 원하나'·" +
  "'# 나에게 끌리는 지점'·'# 나에게 부담인 지점'·'# 반복되는 관계 패턴'·'# 지혜롭게 다루는 법'도 [상대 작동방식] 근거의 구체 항목을 " +
  "최소 하나씩 문장 속에 직접 연결해서 써라 — 다른 사람 리포트에 그대로 옮겨도 말이 되는 문장은 실패로 간주한다. " +
  "전체 분량은 공백 포함 5000~7000자로, 중복 없이 '# 확실 / 추정 / 확인 필요'까지 완결해라.";
...
/**
 * 고급(유료) 리딩에 앵커로 깔아줄 JudgmentPack.
 * 기본과 달리 게이트 라우팅(api/reading.ts)에는 관여하지 않는다 — 고급은 그대로 Content Gate를 쓴다.
 * 이 팩은 "원자료 심화 해석의 결론 방향"이 기본(JudgmentPack)과 어긋나지 않도록 프롬프트에 앵커 블록으로만 주입된다.
 * buildReadingJudgmentPack(기본 전용)과 분리해 둔 이유: 고급에서 non-null 팩이 나오면
 * api/reading.ts가 고급을 Evidence Gate로 오라우팅해 심화 확장이 죽기 때문이다.
 */
function buildAnchorJudgmentPack(facts: ReadingFacts): JudgmentPack | null {
  if (!facts.sajuChart) return null;
  if (facts.type !== "saju" && facts.type !== "combo") return null;
  if (facts.context?.depth !== "advanced") return null;
  const compactEvidence = buildCompactEvidence(facts.sajuChart, facts.luckCycles, facts.gender);
  return buildJudgmentPack({
    readingType: facts.type,
    compactEvidence,
...
export function buildReadingUserMessage(facts: ReadingFacts, prebuiltJudgmentPack: JudgmentPack | null = buildReadingJudgmentPack(facts)): string {
  ...
  if (facts.sajuChart && compactMode) {
    ...
    if (facts.gender) parts.push(`[기본 정보] 성별: ${facts.gender === "male" ? "남성" : "여성"}`);
    // 고급 앵커링: 원자료(아래)로 깊게 확장하되, 결론 방향은 기본과 동일한 JudgmentPack에 앵커링해
    // 같은 사주로 기본/고급을 돌렸을 때 판단이 어긋나는 신뢰 역전을 막는다. 게이트는 그대로 Content Gate.
    const anchorPack = buildAnchorJudgmentPack(facts);
    if (anchorPack) {
      parts.push(formatJudgmentPackForPrompt(anchorPack, "anchor"));
    }
