...
# 직업과 돈
잘 맞는 일의 환경과 오래 버티기 힘든 조건, 돈이 붙는 방식과 새는 구멍, 지금 바꾸면 좋은 습관 하나를 준다.
좋다·나쁘다로 말하지 말고 "어떻게" 그런지 구체적으로. 투자·사업 성패는 단정하지 마라.
반드시 잘 맞는 일의 환경, 피로해지기 쉬운 환경, 돈을 버는 방식, 돈이 새는 패턴,
조직형/프리랜서형/전문직형/사업형 적합성, 현대 직업 예시, 지금 준비하면 좋은 것을 포함한다.
...
# 올해의 흐름
올해를 비슷한 시기끼리 몇 구간으로 나눠, 각 구간의 키워드 / 좋아지는 기회 / 조심할 점 / 한 줄 조언을 준다.
...
// 깊이를 따로 고르지 않은 기본 리딩 — 종합 사주풀이(모든 섹션)이되 간결하게(속도 확보)
const DEFAULT_STANDARD_INSTRUCTION =
  "[기본 리딩 — 종합] 사용자가 깊이를 따로 고르지 않았다. 위 모든 섹션(첫 점괘 / 분야별 요약 / " +
  "타고난 성격과 기질 / 직업과 돈 / 애정과 관계 / 건강과 컨디션 / 인생의 큰 흐름 / 올해의 흐름 / " +
  "지금 해야 할 것과 피해야 할 것 / 마지막 점괘)을 빠짐없이 순서대로 쓰되, 각 섹션을 2~4문장으로 " +
  "간결하게 유지해라. 전체 공백 포함 3000자 이내. 첫 점괘부터 군더더기 없이 바로 시작해라. " +
  "인생의 큰 흐름과 올해의 흐름은 전달된 대운·세운 근거를 반드시 활용하되 용어 없이 쉬운 말로 풀어라.";

const DEPTH_INSTRUCTION: Record<NonNullable<ReadingContext["depth"]>, string> = {
  light:
    "[가벼운 리딩] 첫 점괘 / 분야별 요약 / 타고난 성격과 기질 / 올해의 흐름 / 지금 해야 할 것과 피해야 할 것 / 마지막 점괘 섹션만 출력해라. 각 섹션은 핵심 판단과 바로 할 행동 중심으로 짧고 밀도 있게. 전체 공백 포함 1800~2400자.",
  basic:
    "[기본 리딩] 표준 출력 형식을 모두 따르되, 각 섹션의 내부 소제목은 유지하고 문단은 짧게 쪼개라. 현실 예시와 행동 조언은 2~3개로 압축한다. 전체 공백 포함 2800~3600자.",
  advanced:
    "[고급 리딩] 표준 출력 형식을 모두 따르고, 타고난 성격과 기질·인생의 큰 흐름을 특히 깊게 파고들어라. '지금 해야 할 것과 피해야 할 것'에 앞으로 1개월 행동 계획을 더 촘촘히 넣어라. 전체 분량은 공백 포함 3800~4800자로 하되, 반드시 '마지막 점괘'까지 완결해라.",
  expert:
    "[전문가 리딩] 표준 출력 형식을 모두 따르고, 다음을 더한다: 정곡을 짚는 강도를 최대로 올리되 모든 문장을 근거에 묶고, 인생의 큰 흐름을 시기별로 더 세밀하게 나누고, 올해의 흐름을 달별에 가깝게 구분하고, '지금 해야 할 것과 피해야 할 것'에 3개월 실행 전략을 넣어라. 전체 분량은 공백 포함 4800~5800자 사이로 밀도 있게 쓰되, 같은 말을 반복해 분량을 늘리지 말고 반드시 마지막 섹션까지 완결해라. (여전히 사주 전문용어는 표면 문장에 쓰지 않는다.)",
};
...
const FOCUS_INSTRUCTIONS: Record<Exclude<ReadingFocus, "general">, string> = {
  career: `[관심사] 직업/돈 — '직업과 돈' 섹션의 비중을 키우고 다음을 구체적으로 다뤄라:
