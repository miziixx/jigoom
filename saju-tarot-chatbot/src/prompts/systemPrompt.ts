...
const DEPTH_INSTRUCTION: Record<NonNullable<ReadingContext["depth"]>, string> = {
  light:
    "[가벼운 리딩] 첫 점괘 / 분야별 요약 / 타고난 성격과 기질 / 올해의 흐름 / 지금 해야 할 것과 피해야 할 것 / 마지막 점괘 섹션만 출력해라. 각 섹션은 짧고 밀도 있게.",
  basic: "[기본 리딩] 표준 출력 형식을 모두 따르되, 각 섹션을 간결하게 유지해라.",
  advanced:
    "[고급 리딩] 표준 출력 형식을 모두 따르고, 타고난 성격과 기질·인생의 큰 흐름을 특히 깊게 파고들어라. '지금 해야 할 것과 피해야 할 것'에 앞으로 1개월 행동 계획을 더 촘촘히 넣어라. 전체 분량은 공백 포함 4000자 내외로 하되, 반드시 '마지막 점괘'까지 완결해라.",
  expert:
    "[전문가 리딩] 표준 출력 형식을 모두 따르고, 다음을 더한다: 정곡을 짚는 강도를 최대로 올리되 모든 문장을 근거에 묶고, 인생의 큰 흐름을 시기별로 더 세밀하게 나누고, 올해의 흐름을 달별에 가깝게 구분하고, '지금 해야 할 것과 피해야 할 것'에 3개월 실행 전략을 넣어라. 전체 분량은 공백 포함 5000~6500자 사이로 밀도 있게 쓰되, 같은 말을 반복해 분량을 늘리지 말고 반드시 마지막 섹션까지 완결해라. (여전히 사주 전문용어는 표면 문장에 쓰지 않는다.)",
};
