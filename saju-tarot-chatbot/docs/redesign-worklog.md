...
|---|---|---|---|
| 2026-07-13 | Opus 4.8 | **[엔진 업그레이드] 계산 엔진 심화 2차 — #5·#6·#2 (ADDITIVE ONLY).** #5 궁위 레이어(`PalaceLayer`/`palaces`: 연/월/일/시주 × 삶의 영역·십신·오행), #6 대운-세운 생극합충 세분(`LuckOverlapRefined`/`LuckOverlap.refined`: 천간 오행 생극 5분류 + 지지 합충 구조화), #2 격국 후보 근거(`GyeokgukCandidate`/`gyeokguk.candidates`·`ambiguityReason`: 월지 지장간 후보 점수화·채택근거·애매이유). 기존값·잠금 불변, 854 테스트·tsc·build 클린 | **#7(억부/조후 유파별 병렬)만 남음 — 용신은 이미 억부/조후/궁통보감/통관 병렬 존재, consolidated 필드 추가 여부는 사용자 확인** |
| 2026-07-13 | Opus 4.8 | **[엔진 업그레이드] 계산 엔진 심화 1차 — #4·#3·#1 (ADDITIVE ONLY).** 사용자 요청(우선순위 4→3→1). #4 합충형파해 구조화(`InteractionDetail`/`interactionDetails`, 기존 `interactions` 문자열 함수 미변경, 라벨 일치 회귀 테스트), #3 십이운성 구조화(`TwelveStageDetail`/`twelveStageDetails`+12단계 쉬운 뜻), #1 신강신약 투명화(`StrengthAssessment.transparency`: 억부론 라벨·자리별 가중치·기여 내역·득령·ratio·임계값). 기존 계산값·잠금 테스트 불변, 봇/웹 근거는 chart JSON 직렬화라 새 필드 자동 포함. 849 테스트·tsc·build 클린 | **남은 엔진 요청: #2 격국/용신 경쟁후보 근거·#5 궁위(자리) 해석 레이어·#6 대운-세운 생극합충 세분·#7 유파별 병렬 — 이어서 진행** |
| 2026-07-13 | Opus 4.8 | **[별개 트랙: 텔레그램 봇] 비용 절감 2차 + 호칭 수정.** ① 잡담-하이쿠 분리: `chatModelPolicy.ts`(순수, generalChat+사주용어없음일 때만 값싼 모델, 합/충/형/파/해 단일글자 오탐 제외)+테스트, `teacher.ts` CHAT_MODEL·pickTeacherModel·runStream modelOverride(haiku는 thinking 파라미터 생략), `messageHandler.ts`에서 잡담만 haiku로 라우팅(사주/타로/궁합 해석은 Sonnet 유지). ② 호칭 '누나'→'선생님' 고정: teacher·tarot·secretary 3개 페르소나. 840 테스트·bot tsc·build 클린 | 실제 텔레그램에서 잡담 오탐(사주인데 haiku로 샜는지)·호칭 육안 확인. **사용자 신규 요청: 계산 엔진 심화(합충형파해 구조화·십이운성·신강신약 가중치 투명화 등 7종, 우선순위 4→3→1) — 별도 작업 대기** |
| 2026-07-13 | Fable 5 | **[별개 트랙: 텔레그램 봇] 공부용 정확도 — 근거 점검(fact grounding) 레이어 추가.** 사용자가 봇으로 사주 공부를 하고 싶다고 해서, LLM 답변이 계산 데이터에 없는 신살·간지·십성·격국을 "네 사주에 있다"고 지어내면(할루시네이션) 답변 끝에 ⚠️ 경고 꼬리를 붙이는 additive 감지 레이어 신규. `src/lib/factGrounding.ts`(닫힌 어휘+별칭 매핑: 도화=년살·백호살=백호대살 등, 이론 설명·조건문 제외, 갑자기/상관없다/성격 등 일상어 오탐 방지)+테스트 13개, `bot/teacher.ts` runStream에 groundingEvidence 연결(사주·궁합, 차단 아닌 표시, 점검 실패≠답변 실패). 같은 세션 앞부분: 외부 만세력 대조 케이스 4·6 채움+독립 JDN/五鼠遁 검산(케이스 1~7 일주·시주 전부 일치, `manseIndependentCheck.test.ts` 8개, `external-manse-comparison.md` 갱신, 커밋 `958765d`). 836/836·bot tsc·build 클린 | 실제 텔레그램 왕복으로 경고 꼬리 오탐/미탐 육안 확인, 웹 리딩(고급 Content Gate)에도 같은 점검 적용 검토, 케이스 7 연·월주 웹 실측(네트워크 허용 환경) |
