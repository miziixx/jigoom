...
- **작업 단계:** A·B·C **전부 완료** + §11 "A~C 뒤" 항목 중 **토픽 5종 템플릿 확장(후속 질문 채팅)**과
  **카드 홈 전면 개편(시안 ①)** 완료. **가격 노출은 사용자가 명시적으로 제외 지시**(2026-07-10,
  "가격노출빼고 다 하구 메인올려줘") — 홈 카드/리포트 목록 어디에도 가격 숫자 없음, 의도적. 이번
  세션에서 **사주 입력 폼 + 리딩 결과 카드의 "박스 안에 박스" 중첩 CSS 정리** 완료 (§11 체크리스트
  항목은 아니고 사용자가 스크린샷으로 지적한 별도 UI 폴리시 요청).
- **지금 진행 중:** 없음.
- **직전 세션이 한 일 (이번 세션):**
  - **입력 폼·리딩 결과 카드 중첩 완화**: 사용자가 `/saju` 폼 스크린샷을 보여주며 "박스 안에 박스"처럼
    답답해 보인다고 지적 → 손볼 범위를 "가볍게 CSS만"으로 먼저 확인받음(AskUserQuestion). 원인은
    두 곳: (1) `.form-section`(1/2단계 박스) 안에 `.consultation-panel.optional-settings-panel`
    ("선택 설정"/"분야·말투·해석 깊이")이 테두리+배경을 가진 별도 박스로 또 감싸고 있던 것 —
    `BirthInfoForm`/`ContextPicker`/`TarotSpreadPicker`/`ComboPage`가 전부 공유하는 클래스라 4곳
    동시 적용됨. (2) 리딩 결과의 섹션 카드(`ReadingSectionCard`, `.card reading-section`) 내부에서
    `[한 줄 결론]/[쉬운 풀이]/[왜 그렇게 보는지]` 등 하위 파트(`.reading-part`, `readingBlocks.tsx`)가
    각각 둥근 테두리+배경 박스였던 것 — 한 섹션에 6~7개가 쌓여 카드 안에 카드가 반복되는 모양이었음.
    두 곳 다 CSS만 수정: `.consultation-panel`은 박스→상단 구분선 1줄로, `.reading-part`는
    박스→얇은 점선 구분선(+톤별 색상은 구분선 색으로 유지)으로 바꿈. 구조/섹션 순서/계산 로직/색감
    (한지 팔레트)은 손대지 않음. **실제로 API 키 없이는 리딩 결과 화면을 재현할 수 없어서**, 기존
    `ReadingResult.test.tsx`가 쓰는 `computeSajuChart`/`computeLuckCycles`로 브라우저 밖에서 직접
