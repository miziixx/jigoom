...
### 보류 신살 추가 · ✅ 완료
- **복성귀인**: 일간의 식신 천간이 원국 천간에 드러나면 성립(파생). (근거: 삼명통회, 년간설도 있으나 일간 기준으로 통일)
- **현침살**: 뾰족한 획 글자(갑·신辛/申·묘·오·미)가 **2개 이상** 모이면 1건 집계(낱글자 과다표기 방지).
- **상문살**(년지+2)·**조객살**(년지−2): 원국 지지가 그 자리에 있으면 성립. 고신·과숙과 같은 년지 기준이라 `computeSinsal`에 함께 둠(원국 위치판정 버전).
- 테스트: `sinsalClassic.test.ts` 케이스 추가, `sajuFeatures.test.ts` KNOWN 갱신. 443 통과.
...
- (선택) 상문·조객을 세운(대운/세운) 흐름에도 연동해 "그 해 발동" 표시.
