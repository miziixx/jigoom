...
### Phase D — 삼명통회 신살 확장 · ⬜ 예정
`src/lib/saju.ts` `computeSinsal`에 append + `SINSAL_GLOSS` 항목 추가:
- 태극귀인, 삼기귀인(천상/지하/인중), 복성귀인, 관귀학관, 재고귀인, 현침살, 격각살.
- ⚠️ 상문/조객은 **세운(년지 상대)** 신살 → 원국 `computeSinsal`이 아니라 대운/세운 경로에 넣을 것(원국에 넣지 말 것).
- ⚠️ 복성귀인/재고귀인/현침살은 판본 차이 있음 → 표에 출처 주석.
