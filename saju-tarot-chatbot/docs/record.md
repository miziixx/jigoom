...
### 후속 — 봇 응답 톤을 "사람이랑 채팅하듯" 전면 개선

사용자 요청: "다 그냥 사람이랑 채팅하는것처럼 업그레이드 해줘." 봇이 로봇 같다는 피드백.

가장 기계적이던 부분을 제거·수정(계산·근거·안전 규칙은 불변, **프롬프트/문구 톤만**):

- **비서 모드(`secretary.ts`)**: 강제 템플릿 `DEFAULT_STRUCTURE`("결론:/이유: 1.2.3./오늘 할 일:")와
  의도 고지(`intentDisclosure`, "이건 기획 질문으로 보고 정리할게") **완전 삭제**. MODE_PROMPTS 4종(자기분석/
  기획/글쓰기/판단)을 "친구처럼 공감 먼저 → 말로 풀어 대화" 톤으로 재작성. 사용 안 하게 된
  `INTENT_LABEL`/`DetectedIntent` import 제거.
- **teacher(`teacher.ts`)**: 사주/점성/궁합 시스템 프롬프트에 "친한 친구랑 톡하듯", **상대 말투에 맞추기
  (반말↔존댓말)**, 먼저 짧게 반응 후 본론, 소제목·번호목록 금지·대화체 규칙 강화. 타로 프롬프트도
  "점집 상담사 말고 친구가 카드 봐주듯" 톤 추가.
- **문구(`messageHandler.ts`)**: 기억 저장 응답에서 카테고리 코드·따옴표 제거(`기억해뒀어요 ✅ (userPreference)
...
## 2026-07-14 (3) — 학습모드 딥다이브 톤 조절 ("초등학생도 이해하게 쉽게" 등)

딥다이브가 항상 강의체로만 나오던 걸, 사용자가 원하는 톤·난이도로 말하게 확장.

- `isDeepExplainRequest`(studyMode.ts): "더 자세히" 외에 톤·난이도 지정
  ("초등학생도 이해하게", "쉽게", "비유로", "예시 들어서", "존댓말로/반말로/재미있게 다시")도 트리거.
  톤 단어 + 설명 요청 동사가 함께 있을 때만 잡아 짧은 퀴즈 답(화·정재·신유) 오탐 방지.
- `STUDY_EXPLAIN_SYSTEM`(teacher.ts): "학생이 원하는 톤·난이도에 맞추기"를 다른 모든 규칙보다
  최우선으로. 쉬운 톤이면 전문용어 최소화·일상 비유·표 생략 허용. 근거(교재 사주 실제 값,
  지어내기 금지)만 톤과 무관하게 고정.
- 오답 뒤 힌트도 "원하는 톤으로 보내주세요"로 안내.

878 테스트(트리거 케이스 확장)·tsc·build 클린.
