...
3. **사람처럼 대화하는 톤** — 모든 모드가 "친한 친구랑 톡하듯". 강제 템플릿(`결론:/이유:/오늘 할 일:`)·의도 고지("이건 ~질문으로 보고 정리할게")·소제목/번호목록을 없애고, **상대 말투에 맞춰(반말↔존댓말)** 먼저 짧게 반응한 뒤 말로 풀어서 답합니다. 상태 메시지도 코드 태그 없이 자연스럽게(예: "ㅇㅋ 기억해뒀어요 👌"). 계산·근거·안전 규칙은 그대로.
...
| `/퀴즈` | 배운 개념 복습 문제 |
| `/학습`, `/study` | **사주 학습모드** — 21장 커리큘럼(음양오행~기타신살)을 고정 교재 사주(임진·기묘·갑술·경오)로 학습. 강의→문제 5개→80% 통과제. **압축 강의·출제·채점은 완전 규칙 기반 = LLM 호출 0회.** `/학습 <장번호>`로 점프. 진행 중엔 일반 텍스트가 퀴즈 답으로 처리되고, `패스`=정답 공개, `/학습종료`·"그만"=일시 중단(진도 유지). 학습 중 **"더 설명해줘"**라고 하면 표·비유·사례분기·자주 틀리는 포인트까지 담은 긴 해설을 `askStudyExplain`이 그때 1회만 Claude를 불러 생성(`bot/studyMode.ts`의 `isDeepExplainRequest`/`deepExplainContext`, `bot/teacher.ts`의 `STUDY_EXPLAIN_SYSTEM`) |
| `/진도` | 학습 진도·오답노트 요약 |
