...
- **`AnswerDepth` only has two effective levels: 기본 (depth `undefined`) and 고급 (`"advanced"`).** `"light"`/`"basic"`/`"expert"` were removed from the `AnswerDepth` type entirely (2026-07-09) — they were never selectable in any page UI (only `DepthChoice`/`ContextPicker` ever set `undefined` or `"advanced"`) and had drifted into dead/unreachable branches across `systemPrompt.ts`, `LoadingNotice.tsx`, golden test fixtures, etc. Do not reintroduce string literals for those three; if you need a new depth tier, add it deliberately and wire it through every switch (`DEPTH_INSTRUCTION`, `usesCompactEvidence`, fan-out, `DEPTH_ETA`) rather than leaving one unreachable.
- **All `saju`/`combo` requests fan out**, regardless of depth (`shouldFanOut()` in `readingApi.ts` no longer branches on depth at all) — 기본 writes the same full section structure as 고급 (no content is trimmed for speed, per the "don't reduce information for a short mode" rule below), so it gets the same parallel front/back split to cut wall-clock latency.
- **기본 uses the JudgmentPack Evidence Gate; 고급 uses the raw-evidence Content Gate.** `usesCompactEvidence()` in `systemPrompt.ts` now triggers on depth being unset (기본) instead of the old unreachable `"light"` check — so 기본 saju/combo readings get `[JudgmentPack — 계산됨]` (pre-computed, validated judgments) as their `[근거 데이터]` instead of the raw `[상세 계산 근거 — 사주 원국]`/`[대운/세운/월운/일진]`/`[분야별 사건 신호]` dump, and `api/reading.ts` runs the stricter `streamJudgmentGatedReply` (validates output against the JudgmentPack, rewrites if it fails) instead of the looser `streamContentGatedReply`. 고급 still gets the full raw evidence dump and the simpler content gate, matching the "고급/전문가 모드에서만 세부 근거를 폭넓게 활용한다" line already in `READING_SYSTEM_PROMPT`. **Important**: `composeInnerPsychology()` (지금 마음/타고난 속마음/재료-출력/저울질 4개 심리 엔진) is deliberately *not* gated by this compact/JudgmentPack flag — it always computes all 4 layers for saju/combo regardless of depth, specifically so turning on the Evidence Gate for 기본 doesn't silently strip content (that used to be tied to the same flag when it only ever fired for the unreachable `"light"` value; decoupled 2026-07-09 to keep "기본 doesn't lose information" intact).
