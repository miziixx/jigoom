...
입고/개고(진술축미 창고), 궁합까지 웹앱 계산 엔진에서 뽑은 근거를 그대로 실어 해석합니다.

명령어: `/saju` 원국 요약(API 호출 없음) · `/today` 오늘 일진 상세 풀이 · `/궁합` 상대와 궁합 보기 · `/퀴즈` 배운 개념 복습 · `/birth` 재등록 안내 · `/reset` 대화 초기화 · `/delete` 데이터 전체 삭제
